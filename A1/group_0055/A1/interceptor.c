...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    // if monitored == 0, skip to end and call original syscall
    ...
    //this calls the function pointer..?  Double check this!
    return (*(table[syscall].f))(reg);
}


/**
 * These four functions are the implementation of each command in my_syscall after error checks
 * They all return their relevant error integer returning 0 if there were no problems
 */
 
// CMD == REQUEST_START_MONITORING
int start_monitor(int pid, int syscall) {
	int res;
	// if root simply set monitored to 2
	if (pid == 0) {
		spin_lock(&my_table_lock);
		...
		// if monitored is 2, all pids are already monitored
...
int stop_monitor(int pid, int syscall) {
	int res;
	if (pid == 0) {
		// if root, destroy pid list if monitored = 1, else simply set monitored to 0
	...
	if (table[syscall].monitored == 2) {
		return -EINVAL;  // THIS IS THE SPECIAL CASE.
	}
	else { // monitored == 1, 0 is checked above
		spin_lock(&my_table_lock);
...
int start_intercept(int syscall) {
	spin_lock(&my_table_lock);
...
int error_check(int cmd, int syscall, int pid) {
	// This block is just for raising errors
	if (syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls) {
		...
		if (pid < 0) {
			...
			if (!(pid_task(find_vpid(pid), PIDTYPE_PID))) { // Check if valid PID with null check
				return -EINVAL;
			}
		}
		if (current_uid() != 0) { // if root no problem but if not..
			if (pid == 0) {
				return -EPERM;
			}
			if (check_pids_same_owner(pid, current->pid) != 0) {
				// synonymous to returning -EPERM
				return check_pids_same_owner(pid, current->pid); 
			}
		}
	}
	...
	// This block checks when monitoring it's not already monitored,
	// or not intercepted to start intercepting (Or vice versa)
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	int error_result;
	// Checks for potential errors with arguments or validity (e.g permissions)
	error_result = error_check(cmd, syscall, pid);
	
	// If result was non-zero (an error) return relevant error.
	if (error_result != 0) {
		return error_result;
	}
	
	// After error-checking, implementation of each command below
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		return start_intercept(syscall);
	}
	
	else if (cmd == REQUEST_SYSCALL_RELEASE) {
		return deintercept(syscall);
	}
	
	else if (cmd == REQUEST_START_MONITORING) {
		return start_monitor(pid, syscall);
	}
	
	else if (cmd == REQUEST_STOP_MONITORING) {
		return stop_monitor(pid, syscall);
	}
...
static int init_function(void) {
	...
	// store custom syscall and exit group, replace with our own
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
...
{   
	int i;
	int t;
	// Deintercept all relevant syscalls
    for(i = 0; i < NR_syscalls+1; i++){
		if (table[i].intercepted == 1) {
			t = my_syscall(REQUEST_SYSCALL_RELEASE, i, current->pid);
			spin_lock(&my_table_lock);
			destroy_list(i);
			spin_unlock(&my_table_lock);
		}
    }
	// Restore custom syscall and exit group
	spin_lock(&sys_call_table_lock);
    set_addr_rw((unsigned long)sys_call_table);
