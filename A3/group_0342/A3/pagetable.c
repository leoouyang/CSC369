...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// if it is dirty or it hasn't been swapped before, it need to be written into swapfile
		if((coremap[frame].pte->frame & PG_DIRTY) || !(coremap[frame].pte->frame & PG_ONSWAP)){
			tempoff = swap_pageout(frame, coremap[frame].pte->swap_off);
			// error check, if swap_pageout return error, terminate the program
			if (tempoff == INVALID_SWAP){
				exit(1);
			}
			coremap[frame].pte->swap_off = tempoff;
			evict_dirty_count++;
		}else{
			evict_clean_count++;
		}
		...
		coremap[frame].pte->frame = coremap[frame].pte->frame & (0x0000000f);
		coremap[frame].pte->frame = coremap[frame].pte->frame & ~PG_DIRTY;
		coremap[frame].pte->frame = coremap[frame].pte->frame & ~PG_REF;
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	uintptr_t pde; // the pde as int
	pgtbl_entry_t *pdepointer; // the pde as pointer(or array)
	unsigned idx = PGDIR_INDEX(vaddr); // get index into page directory
	unsigned itx = PGTBL_INDEX(vaddr); // get index into second level page table
	int frame = 0;
	int errorcheck;
	...
	// Initialize the second level table if it is not initialized
	if(!(pde & PG_VALID)){
	...
	pdepointer = (pgtbl_entry_t *)(pde & PAGE_MASK);
	p = &pdepointer[itx];

	// Check if p is valid or not, on swap or not, and handle appropriately
	if(!(p->frame & PG_VALID)){
		// If it is not valid and not on swap, then it is a new reference and a new frame is allocated to it
		if(!(p->frame & PG_ONSWAP)){
			frame = allocate_frame(p);
			p->frame = (frame << PAGE_SHIFT) | p->frame;
			// save the vaddr to physmem(corresponded frame)
			init_frame(frame, vaddr);
		}
		// If the frame is swapped out, allocate a new frame to it and swap it in
		else{
			frame = allocate_frame(p);
			p->frame = (frame << PAGE_SHIFT) | p->frame;
			// save the vaddr to physmem(corresponded frame) from swap file
			errorcheck = swap_pagein(frame, p->swap_off);
			if (errorcheck != 0){  //if error, terminate the program
				exit(1);
			}
		}
		miss_count++;
